...
- [x] Créer application Streamlit (`src/ui/web_app.py`)
- [x] Implémenter layout 3 colonnes (gauche, centre, droite)
- [x] Implémenter bandeau haut (expander scénario, variabilité, bloc ML 3×2)
- [x] Implémenter bandeau bas (jours, progression, boutons)
- [x] Implémenter colonne gauche (liste événements/incidents)
- [x] Implémenter colonne centre (carte Paris avec 100 microzones)
- [x] Implémenter colonne droite (rectangles arrondissements)
- [x] Charger données 100 microzones depuis pré-calculs Epic 1
- [x] Implémenter widgets Streamlit (selectbox, slider, button, radio)
- [x] Appliquer charte visuelle (pastel, Inter, français, emojis)
...
| Fichier | Action |
|---------|--------|
| `src/ui/__init__.py` | Créé |
| `src/ui/web_app.py` | Créé |

---

## Completion Notes

- Application dans `src/ui/web_app.py` (AC1 : chemin indiqué dans les critères d’acceptation).
- Carte : chargement via `PathResolver.data_source("microzones.pkl")`, support format brut ou `{ "data": gdf }`.
- Lancement : `streamlit run src/ui/web_app.py` (ou `python -m streamlit run src/ui/web_app.py`) depuis la racine du projet, avec l’environnement contenant streamlit/folium/geopandas activé (ex. `conda activate paris_risques`).
...
| 29 Jan 2026 | 1.1 | Implémentation layout Streamlit, 3 colonnes, bandeaux, carte 100 microzones | Dev |
